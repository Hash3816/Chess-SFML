#include "../main_structure_and_logic/chess_board.h"
#include "../main_structure_and_logic/pieces.h"
...
bool Chess_Board::make_castling(Type_Castling type_castling, Color color_current_player) { //true - если рокировка произведена, false если нет
    if (type_castling == Type_Castling::Long && color_current_player == Color::White) {
        ...
        if (this->is_check(color_current_player)) {//Если после хода игрок попадает под мат, возращаем прошлое состояние доски обратно
